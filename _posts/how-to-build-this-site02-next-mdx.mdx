...
And create `_posts` directory to place all post write by mdx, and there two mdx file as posts.
...
To add content in our blog home page, which is in `index.tsx`, to list all the posts.
...
But if we need pagination used in post list by query parameter, like `http://localhost:3000/blog?page=1`, we could not get the query param in `getStaticProp`. Because it's only invoke on a build time, and not refresh when query parameters are changed.

Next.js provide other method `GetServerSideProps`, it's primarily used to fetch data for every time when user with different request, like query paramters are changed.
